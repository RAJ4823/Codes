...
/*
    ...
    *Step 4 − If the scanned character is the operator then check.
        ?a) If the Stack is empty, push it into the Stack.
        ?b) If priority of the scanned operator is higher than the operator of the stack, push it into the stack.
        ?c) If priority of the scanned operator is lower than the operator of the stack, pop and set it in prefix expression.
        ?d) Repeat Step 4(d) till it finds the operator of a lower or same priority.
        !Optional)If the scanned operator has the same priority with the operator in the stack and the incoming operator is ^, then pop the top of the stack till the condition is true. If the condition is not true, push the ^ operator.
