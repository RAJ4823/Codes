// ⭐ the_247 | Raj Patel ⭐
#include <bits/stdc++.h>
using namespace std;

#define ll long long
#define pb push_back
#define ff first
#define ss second
#define vll vector<ll>
#define vvll vector<vll>
#define pll pair<ll, ll>
#define vpll vector<pll>
#define pqll priority_queue<ll>
#define umll unordered_map<ll, ll>
#define usll unordered_set<ll>
#define mll map<ll, ll>
#define sll set<ll>

#define fauto(i, a) for (auto &i : a)
#define f(i, s, e) for (int i = s; i <= e; i++)
#define fr(i, s, e) for (int i = s; i >= e; i--)

#define all(x) (x).begin(), (x).end()
#define sortv(v) sort(all(v))
#define sortvd(v) sort(all(v), greater<>());
#define sortstr(str) sort(all(str))
#define sortstrd(str) sort(all(str), greater<char>())
#define tolowerstr(str) transform(all(str), str.begin(), ::tolower)
#define toupperstr(str) transform(all(str), str.begin(), ::toupper)
#define print_condition(x, s1, s2) cout << ((x) ? s1 : s2) << endl;
#define print_vector(v) fauto(x, v) cout << (x) << ' ';

#define db(x) cout << (#x) << " = " << x << "\n";
#define dbb(x, y) cout << (#x) << " = " << x << " , " << (#y) << " = " << y << "\n"

#define PI 3.1415926535897932384626433832795
#define MOD 1000000007LL
#define MODD 998244353
#define INF LONG_LONG_MAX

ll find(ll i, vvll &graph, vll &coins, vll &minCost)
{
    if (minCost[i] != INF)
        return minCost[i];
    if (graph[i].size() == 0)
        return min(minCost[i], coins[i]);

    ll sum = 0;
    fauto(j, graph[i]) sum += find(j, graph, coins, minCost);

    return minCost[i] = min({minCost[i], coins[i], sum});
}
int main()
{
    int t;
    cin >> t;

    while (t--)
    {
        ll n, k;
        cin >> n >> k;
        vll coins(n + 1), portion(k + 1), minCost(n + 1, INF);
        vvll graph(n + 1);

        f(i, 1, n) cin >> coins[i];
        f(i, 1, k)
        {
            cin >> portion[i];
            coins[portion[i]] = 0;
        }

        f(i, 1, n)
        {
            ll m;
            cin >> m;
            f(j, 1, m)
            {
                ll x;
                cin >> x;
                graph[i].push_back(x);
            }
        }

        f(i, 1, n) if (minCost[i] == INF) minCost[i] = find(i, graph, coins, minCost);
        f(i, 1, n) cout << minCost[i] << ' ';
        cout << endl;
    }
    return 0;
}